...
export default function Dashboard() {
  ...
  if (loading) return (
    <div className="min-h-screen bg-surface flex items-center justify-center text-white gap-2">
      <Loader2 className="animate-spin text-accent" /> Loading Dashboard...
    </div>
  );

  return (
    <div className="min-h-screen p-4 md:p-8 bg-surface text-white">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10">
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              <span className={`px-2 py-0.5 rounded text-xs font-bold uppercase ${tier === 'pro' ? 'bg-accent/20 text-accent' : 'bg-white/10 text-white/50'}`}>
                {tier} Plan
              </span>
          ...
          <div className="flex gap-3">
             ...
             <button 
               ...
               className="flex items-center gap-2 px-4 py-3 bg-white/5 hover:bg-white/10 text-white rounded-xl font-bold transition-all disabled:opacity-50 border border-white/10"
             >
             ...
             <button 
               ...
               className={`flex items-center gap-2 px-6 py-3 rounded-xl font-bold transition-all shadow-[0_0_15px_-5px_var(--color-accent)] ${
                 canCreate 
                  ...
                  : 'bg-white/5 text-white/40 cursor-not-allowed shadow-none border border-white/10'
               }`}
             >
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            >
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="text-sm text-white/50">Created {new Date(tree.created_at).toLocaleDateString()}</p>
                </div>
                
                {/* DELETE BUTTON */}
                <button 
                  onClick={(e) => handleDelete(e, tree.id)}
                  className="p-2 bg-white/5 hover:bg-red-500/20 text-white/30 hover:text-red-400 rounded-lg transition-colors z-20"
                  title="Delete Tree"
                >
          ...
          {trees.length === 0 && (
            <div className="col-span-full py-24 text-center border-2 border-dashed border-white/10 rounded-2xl bg-white/5">
              <TreeDeciduous className="w-12 h-12 text-white/20 mx-auto mb-4" />
              <h3 className="text-xl font-bold text-white/40">No trees yet</h3>
              <p className="text-white/30 mb-6">Create your first tree manually or import a file.</p>
              <button onClick={handleCreate} className="px-6 py-2 bg-white/10 hover:bg-white/20 rounded-lg text-white font-medium transition-colors">
                Create Now
              </button>
            </div>
          )}
